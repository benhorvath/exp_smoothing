...
## $ES_{3}$: Exponential weighted average
...
```{r}
# TODO: Fix this up: Need to incorporate T and y and h parameters, but
# the math looksfine
ewa <- function(i, T, alpha) alpha * (1 - alpha)^i * y[T - i] + (1 - alpha)^T * y[1]

# T <- length(y)
# ( y_hat <- sum(sapply(1:T-1, ewa, T=T, alpha=0.5 )) )
```
